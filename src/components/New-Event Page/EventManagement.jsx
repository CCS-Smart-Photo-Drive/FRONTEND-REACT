...
const EventManagement = ({ tasks = [], onAddTask, removeTask }) => {
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [selectedEvent, setSelectedEvent] = useState(null);
  const [isDownloading, setIsDownloading] = useState(false);
  ...
  const handleSubmit = async (eventData) => {
    ...
    if (Array.isArray(eventData.images)) {
      ...
      const imageFiles = eventData.images.map(file => file.file);
      const zipFile = await zipImages(imageFiles);
      processedEventData.images = zipFile;
    }
    
    onAddTask(processedEventData);
  };
      ...
      {selectedEvent && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
          <div className="bg-black p-8 rounded-xl shadow-xl w-full max-w-md">
            ...
            <div className="flex items-center justify-between mb-4">
              <p className="text-gray-300">
                <strong>File:</strong> {selectedEvent.file ? selectedEvent.file.name : "No file uploaded"}
              </p>
              {selectedEvent.fileContent && (
                <button
                  onClick={() =>
